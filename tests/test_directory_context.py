...
class TestCourseContext(TestCase):
    ...
    def test_get_directory_info2(self):
        """
        Happy path test for a known contact
        """
        self.context.query = "Jeffrey Graves"
        self.context.get_directory_info()
        self.assertEqual("jgraves@tntech.edu", self.context.entries[0].email)
    ...
    def test_get_info_multiple2(self):
        """
        Happy path test for query with multiple entries
        """
        self.context.query = "Graves"
        self.context.get_directory_info()
        self.assertEqual(1, len(self.context.entries))
    ...
    def test_get_info_large2(self):
        """
        Happy path test for query with multiple entries
        """
        self.context.query = "David"
        self.context.get_directory_info()
        self.assertEqual(16, len(self.context.entries))
    ...
    def test_get_no_data2(self):
        """
        Tests whether the input contact
        """
        self.context.query = "zxcvbn;"
        with self.assertRaises(DirectoryNotFoundException) as ex:
            self.context.get_directory_info()
